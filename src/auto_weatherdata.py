...
for grid_lat, grid_lon, grid_id in grid_cells:
    ...
    for attempt in range(3):
        try:
            ...
            logger.error(f"  ❌ Failed on attempt {attempt+1}: {e}")
            time.sleep(5)
    else:
        logger.warning(f"  ⚠️ Skipping after 3 failed attempts")
        continue
...
if all_weather_data:
    ...
    logger.warning(f"\n⚠️ No weather data fetched from {start_date.isoformat()} to {end_date.isoformat()}!")